...
export function HomeDashboard({ accountNotice = null, accountNoticeUpdatedAt = null }: { accountNotice?: string | null; accountNoticeUpdatedAt?: string | null }) {
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <ul className="mt-4 flex items-center justify-between">
          {week.map((day) => (
            <li key={day.key} className="flex flex-col items-center gap-2">
              <span className={`flex h-9 w-9 items-center justify-center rounded-full border text-xs font-semibold transition-colors ${day.active ? "border-transparent bg-brand text-brand-foreground" : day.today ? "border-dashed border-brand text-brand" : "border-border text-muted-foreground"}`}>
      ...
      <Link href="/shop" className="app-surface press rise-in flex items-center gap-4 rounded-3xl border border-border bg-card p-5" style={rise(4)}>
        <Weaver size={52} />
      ...
      {latest && (
        <Link href={`/coach?recording=${latest.id}`} className="app-surface press rise-in flex items-center gap-4 rounded-3xl border border-border bg-card p-5" style={rise(5)}>
      ...
      <Link href="/coach" className="app-surface press rise-in flex items-center gap-4 rounded-3xl border border-border bg-card p-5" style={rise(6)}>
