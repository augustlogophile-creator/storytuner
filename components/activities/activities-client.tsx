...
import { courseProgress, isUnitUnlocked, unitProgress, useApp } from "@/lib/app-state"
  ...
  return (
    <div className="flex flex-col gap-6">
      <header>
        ...
        <h1 className="mt-2 text-2xl font-semibold tracking-tight text-balance">The craft of true storytelling</h1>
      ...
      <section className="rounded-3xl border border-border bg-card p-5">
        <div className="mb-2 flex items-center justify-between text-sm">
          ...
          <span className="text-muted-foreground">{course.done} of {course.total} steps</span>
        </div>
      ...
      <div className="flex flex-col gap-4">
        {curriculum.map((unit) => {
          ...
          const content = (
            <article className={cn("group flex flex-col gap-4 rounded-3xl border bg-card p-5 transition-colors", unlocked ? "border-border hover:border-brand/50" : "border-border/70 opacity-70")}>
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0 flex-1">
                  <p className="font-mono text-[0.65rem] uppercase tracking-[0.16em] text-muted-foreground">{unit.kind === "capstone" ? "Capstone" : unit.skill}</p>
                  <h2 className="mt-1 text-base font-semibold text-foreground">{unit.title}</h2>
          ...
          return unlocked && ready ? <Link key={unit.id} href={`/activities/${unit.id}`}>{content}</Link> : <div key={unit.id}>{content}</div>
        })}
      </div>
    </div>
  )
