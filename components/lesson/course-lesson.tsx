    ...
    return (
      <div className="flex min-w-0 flex-col gap-5">
        ...
        <section className="rounded-3xl border border-brand/30 bg-brand-soft/35 px-6 py-10 text-center">
          ...
          <Link href="/membership?from=lessons" className="mt-5 flex items-center justify-center gap-2 rounded-full bg-primary px-5 py-3.5 text-sm font-semibold text-primary-foreground">See Membership<ArrowRight className="h-4 w-4" /></Link>
        </section>
      </div>
    )
  }
...
function DrillStage({ unit, response, setResponse, onFinish }: { unit: CurriculumUnit; response: string; setResponse: (value: string) => void; onFinish: (value: string, xp: number) => void }) {
  ...
  async function getFeedback() {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "Parch could not respond right now.")
      setFeedback(null)
    } finally {
      setLoading(false)
    }
    ...
    <>
      ...
      {feedback && (
        <section className="lesson-feedback-enter flex flex-col gap-3 rounded-3xl border border-brand/40 bg-brand-soft/35 p-5">
          ...
          <ReportAiOutput source="practice" content={`${feedback.working}\n\n${feedback.fix}`} lessonId={unit.id} />
        </section>
      )}
      {!feedback ? (
