...
The practice mode and exercise prompt are part of the grading context. When the exercise prompt is specific rather than an open-ended "tell your own story" prompt, explicitly judge whether the speaker actually answered that prompt, stayed on topic, and made the response interesting through specific relevant detail. Reflect prompt adherence mainly in the development score and in at least one strength or improvement when it is meaningfully strong or weak. Do not penalize an open-story response for prompt adherence, and never reward mere keyword matching over a coherent story.
