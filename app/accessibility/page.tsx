...
export default async function AccessibilityPage({ searchParams }: { searchParams?: LegalSearchParams }) {
  ...
  return (
    <LegalPage
      ...
      title="Accessibility"
      summary="Tellwise aims to make its core learning, recording, account, and Community experiences usable with keyboard, touch, zoom, and assistive technology, as best as we can."
    >
      <LegalSection title="Current approach">
        <p>Tellwise uses semantic headings and controls, visible keyboard focus, text labels for interactive controls, mobile-safe touch targets, responsive layouts, and support for browser text zoom.</p>
      ...
      <LegalSection title="Feedback">
        <p>If a Tellwise feature is difficult to use with a screen reader, keyboard, switch device, voice control, zoom, or another assistive technology, please email <a className="underline underline-offset-4" href="mailto:tellwiseapp@gmail.com">tellwiseapp@gmail.com</a>. Include the page and what you were trying to do so the issue can be reproduced and fixed.</p>
      </LegalSection>
